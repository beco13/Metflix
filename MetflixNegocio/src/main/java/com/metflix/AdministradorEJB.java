...
public class AdministradorEJB implements AdministradorEJBRemote {
	...
	/**
	 * Permite consultar el número de peliculas compradas por un cliente en un intervalo de tiempo dado
	 * 
	 * @param clienteId
	 * @param fechaInicio
	 * @param fechaFin
	...
	public int findPeliculasVendidasIntervalo(int clienteId, Date fechaInicio, Date fechaFin ) {
		Query query = em.createQuery("select count(*) as total_peliculas_compradas from pelicula_venta PV left join venta V on PV.ventas_ID = v.ID where V.CLIENTE_ID = :cliente_id and V.fecha between :fechaInicio AND :fechaFin ");
	...
	/**
	 * 
	 * Permite buscar un administrador por el correo y luego 
	 * le envia la contraseña al email que el administrador tenga registrado
	 * 
	 * 
	 * @param correo
	 * @return
	...
	public int recuperarPassword(String correo) {
		...
		tmpCorreo.setMensaje("Has solicitado recuperar la contraseña para lo cual esta es tu contraseña: "+administrador.getContrasena());
